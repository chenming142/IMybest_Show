	...
	function(IMybest_Show) {
		function calculate(width, height, w, h) {
			var dimension = {},
				rate = width / height,
				pRate = w / h;
			rate > pRate ? (dimension.width = w, dimension.height = w / rate) : (dimension.height = h, dimension.width = h * rate);
			return dimension;
		}
		...
		var JSONParser = IMybest_Show.templateParser("jsonParser", function() {
			...
			function wrapComponent(comp, mode) {
				...
				if (component) {
					...
					5 != ("" + comp.type).charAt(0) && 6 != ("" + comp.type).charAt(0) || "edit" != mode || $(component).before($('<div class="element" style="position: absolute; height: 100%; width: 100%;">'));

					if (comp.css) {
						element.css({
							width: 320 - parseInt(comp.css.left)
						});
						element.css({
				...
				Parser = {
					...
					parse: function(sceneTpl) {
						var wrap = $('<div class="edit_wrapper">'
                                        + '<ul id="edit_area' + sceneTpl.def.id + '"' + ' comp-droppable paste-element ' + 'class="edit_area weebly-content-area weebly-area-active"></ul>'
                                    + '</div>'),
							mode = this.mode = sceneTpl.mode;
						this.def = sceneTpl.def;
                        if("view" == mode)p++;
		...
		JSONParser.addInterceptor(function(wrapComponent, element) {
			...
			if (element.properties && element.properties.anim) {
				...
				anim.trigger ? wrapComponent.click(function() {
		...
		JSONParser.addComponent("3", function(component) { //bg
			...
			if (component.properties.imgSrc) {
				element.style.backgroundImage = /^http.*/.test(component.properties.imgSrc) ? "url(" + component.properties.imgSrc + ")" : "url(" + PREFIX_FILE_HOST + "/" + component.properties.imgSrc + ")";
		...
		JSONParser.addComponent("4", function(component) { //normal img
			...
			element.src = /^http.*/.test(component.properties.src) ? component.properties.src : PREFIX_FILE_HOST + component.properties.src;
			return element;
		});
		...
		JSONParser.bindAfterRenderEvent("1", function(wrap, component) {
			...
			if ("view" == JSONParser.mode && 1 == component.type) {
				var labels = component.properties.labels,
					label;
				for (label in labels) {
					! function(l) {
