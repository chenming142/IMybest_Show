...
! function(a, b, c) {
    ...
    b.module("app").controller("AppCtrl", ["SpreadService", "$window", "$scope", "$rootScope", "$location", "$modal", "security", "sceneService", "$routeParams", "$timeout", "i18nNotifications",
        function(a, b, c, d, e, f, g, h, i, j, k) {
            c.notifications = k,
            c.removeNotification = function(a) {
                k.remove(a)
            },
            ...
            if (m.test(n)) {
                var o = n.split("=");
                c.weiboAccessToken = o[1].split("&")[0],
                c.weiboRemindIn = o[2].split("&")[0],
                c.weiboExpires = o[3].split("&")[0],
                c.weiboUId = o[4].split("&")[0]
            ...
            c.isAuthenticated = g.isAuthenticated,
            c.$watch(function() {
                ...
                b && (
                    c.user = b,
                    d.user = b,
                    c.isEditor = g.isEditor(),
                    d.isEditor = g.isEditor(),
                    c.isAdvancedUser = g.isAdvancedUser(),
                    d.isAdvancedUser = g.isAdvancedUser(),
                    c.isVendorUser = g.isVendorUser(),
                    d.isVendorUser = g.isVendorUser(),
                    a.getActivityDetail("001").then(function(a) {
                        var b = a.data.obj;
                        if (b) {
                            d.sendXd = b;
            ...
            c.login = function() {g.showLogin()},
            c.register = function() {g.showRegister()},
            c.showToolBar = function() {return e.$$path.indexOf("/scene/create") >= 0 ? !1 : !0},
            ...
            c.suggestionUrl = "http://eqxiu.hjtmt.com/forum.php?mod=forumdisplay&fid=45",
            c.feedbackUrl = "http://eqxiu.hjtmt.com/forum.php?mod=forumdisplay&fid=46",
            c.qqChatUrl = "http://shang.qq.com/wpa/qunwpa?idkey=4a2d63670009360b878aa9a1e1437ef4caec132f74a0e2c4df4a686168cc73dc",
            c.helpUrl = "http://eqxiu.hjtmt.com/forum.php", c.createSkillUrl = "http://eqxiu.hjtmt.com/forum.php?gid=37"
    ...
    b.module("scene.create").controller("CreateSceneCtrl", [
        "$timeout", "$compile", "$rootScope", "$scope", "$routeParams", "$route",//end h
        "$location", "sceneService", "pageTplService", "$modal", "ModalService", "security",
        "$window", "i18nNotifications", "historyService",
        ...
        function t(a, c, d) {
            ...
            f.loading = !1,
                ...
                f.tpl.obj.properties && (f.tpl.obj.properties.image || f.tpl.obj.properties.scratch)
                    ? (
                        f.tpl.obj.properties.scratch
                            ? f.scratch = f.tpl.obj.properties.scratch
                            : f.tpl.obj.properties.image && (
                                f.scratch.image = f.tpl.obj.properties.image,
                                f.scratch.percentage = f.tpl.obj.properties.percentage,
                                f.tpl.obj.properties.tip && (f.scratch.tip = f.tpl.obj.properties.tip)
                            ),
                        f.effectName = "涂抹",
                        b.forEach(f.scratches, function(a) {
                ...
                a.value == f.scratch.percentage.value && (f.scratch.percentage = a)
                    ...
                    : (
                        ...
                        f.scratch.percentage = f.percentages[0]), f.tpl.obj.properties && f.tpl.obj.properties.finger ? (f.finger = f.tpl.obj.properties.finger, f.effectName = "指纹", b.forEach(f.fingerZws, function(a) {
        ...
        f.saveScene = function(a, c) {
            if (!C) {
                if (C = !0, x == JSON.stringify(f.tpl)) return c && c(),
                a && (
                    !f.tpl.obj.scene.publishTime || f.tpl.obj.scene.updateTime > f.tpl.obj.scene.publishTime
                        ? u()
    ...
    b.module("services.scene").factory("sceneService", ["$http", "$rootScope", "$modal", "$q", "security", "$cacheFactory", "historyService",
        ...
        function x(a) {
            z(a, function(b) {
                ...
                "backgroundColor" == b.type && (
                    c.style.backgroundImage = "none",
                    c.style.backgroundColor = b.color,
                    a.properties.imgSrc = null, a.properties.bgColor = b.color),
                h.addPageHistory(G.obj.id, G.obj.elements),
                $("#editBG").unbind("click"),
        ...
        return F.addInterceptor(function(a, e, g) {
            function i() {
                ...
                return q && b.find(".copy").after($('<li class="paste" role="presentation"><a role="menuitem" tabindex="-1"><div class="fa fa-paste" style="color: #08a1ef;"></div>&nbsp;&nbsp;粘贴</a></li>')),
                    b.find(".edit").click(function(c) {
                    ...
                    4 != e.type && (b.find(".link").hide(), b.find(".cut").hide()),
                    "p" == e.type && (b.find(".animation").hide(), b.find(".style").hide()), b
            }
            if ("view" != g) {
                var j = $("#eq_main");
